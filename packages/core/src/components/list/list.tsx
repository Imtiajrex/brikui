...
function getContentPadding(
	...
	if (axis === "y") {
		const start =
			pickStyleNumber(flat, "paddingTop", "paddingVertical", "padding") ?? 0;
		const end =
			pickStyleNumber(flat, "paddingBottom", "paddingVertical", "padding") ?? 0;
		return { start, end };
	}

	const start =
		pickStyleNumber(flat, "paddingLeft", "paddingHorizontal", "padding") ?? 0;
	const end =
		pickStyleNumber(flat, "paddingRight", "paddingHorizontal", "padding") ?? 0;
	return { start, end };
}
...
const List = React.forwardRef<LegendListRef, LegendListProps<any>>(
	function List(props, forwardedRef) {
		...
		React.useImperativeHandle(
			forwardedRef,
			() => internalRef.current as LegendListRef,
			[]
		);
		...
		const wrappedRenderItem = React.useMemo(() => {
			...
			const render =
				typeof renderItem === "function"
					...
					: (info: LegendListRenderItemProps<any, any>) =>
							React.createElement(renderItem, info);
